    ...
    return(
        <div>
            ...
            <label className={`${birth.id}_edit hidden`}>Birth Year: <input className={`${birth.id}_edit birth_inputs hidden`} type="number" name="birth-birth_year"  onChange = {handleChange}/></label>
                ...
                <label>Examined By: {birth.examiner_name} <input name="birth-examiner_name" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                ...
                <label>Delivery Time: {new Date(birth.delivery_time).toString().split(" ")[4].slice(0,5)} <input type="time" name="birth-delivery_time" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                ...
                <label>Weight: {birth.weight ? birth.weight : "0"} kg<input type="number" step="0.01" name="birth-weight" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                <label>Length: {birth.length ? birth.length : "0"} cm<input type="number" step="0.01" name="birth-length" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                <label>Head Circumference: {birth.head_circumference ? birth.head_circumference : "0"} cm<input type="number" step="0.01" name="birth-head_circumference" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                <label>Estimated Gestation: {birth.estimated_gestation} weeks<input type="number" name="birth-estimated_gestation" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                ...
                <label>{birth.newborn_bloodspot_screening_test_completed ? "Bloodspot Date: " + new Date(birth.bloodspot_sample_date).toString().slice(0,10) : null}<input type="date" name="birth-birth.bloodspot_sample_date" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                <label>Apgar One Minute: {birth.apgar_one_minute}<input type="number" name="birth-apgar_one_minute" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                <label>Apgar Five Minute: {birth.apgar_five_minute}<input type="number" name="birth-apgar_five_minute" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                <label>Problems Requiring Treatment: {birth.problems_requiring_treatment ? birth.problems_requiring_treatment : "None"}<input type="string" name="birth-problems_requiring_treatment" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                ...
                <label>Intensive Care Reason:{birth.admission_to_intensive_care_nursery_48hours ? `${birth.intensive_care_reason}` : " N/A"}<input name="birth-intensive_care_reason" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
                ...
                <label>Intensive Care Reason: {birth.admission_to_special_care_nursery_48hours ? `${birth.special_care_reason}` : " N/A"}<input name="birth-special_care_reason" className={`${birth.id}_edit birth_inputs hidden`} onChange = {handleChange}/></label>
